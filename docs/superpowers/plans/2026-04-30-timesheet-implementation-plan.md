...
| 审批方式 | 统一收归模块3「审批入账」→ **3.2 成本审批**（独立功能页面） | 工时填报仅负责「提交审批」，审批人在 3.2 成本审批页面统一处理工时单和报销单 |
...
> 审批操作（通过/驳回/入账）统一在模块3.2「成本审批」中完成。工时详情页仅用于查看，不内嵌审批按钮。
...
| 工时审批 | `project:workHour:approve` | 模块3.2「成本审批」中使用 |
| 工时入账 | `project:workHour:post` | 模块3.2「成本审批」中使用 |
...
**审批流程（全部在模块3.2「成本审批」中完成）**：
1. 提交审批：DRAFT/REJECTED → PENDING（工时填报页面操作）
2. 审批通过：PENDING → APPROVED（模块3.2操作）
3. 驳回：PENDING → REJECTED，必填原因（模块3.2操作）
4. 入账：APPROVED → POSTED，扩展预算扣减（模块3.2操作）
